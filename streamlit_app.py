...
# --- KONFIGURACIJA ---
st.set_page_config(page_title="NatGas Sniper V105 - Master", layout="wide")
...
# --- PERSISTENCE ENGINE (Ironclad UTF-8) ---
DATA_FILE = "sniper_v105_master.json"
...
with col_m:
    ...
    if curr_mx:
        ...
        gtd = gc - gp
        st.markdown(f"<div class='grand-total-box'><h4>GRAND TOTAL PW-HDD | Model Run: <strong>{run_tag}</strong></h4><h1 style='margin:10px 0; font-size:3rem; color:#008CFF;'>{gc:.2f} <span class='{'bull-text' if gtd > 0 else 'bear-text'}' style='font-size:1.5rem;'>({gtd:+.2f})</span></h1></div>", unsafe_allow_html=True)

    if st.button("💾 SPREMI MODEL KAO REFERENTNU BAZU"):
        st.session_state.data["last_hdd_matrix"] = curr_mx
        save_data(st.session_state.data); st.rerun()

    # 2. AI STRATEGIC ANALYST (Groq Master)
    st.subheader("🤖 Neural Strategic Analyst")
    if st.button("🚀 POKRENI ANALIZU ASIMETRIJE"):
        if not groq_key:
            ...
            try:
                ...
                p = (f"Analyze: EIA {st.session_state.data['eia_curr']} Bcf, Deficit vs 5y {st.session_state.data['eia_curr']-st.session_state.data['eia_5y']} Bcf. "
                     f"COT MM Net {st.session_state.data['mm_l']-st.session_state.data['mm_s']}. "
                     f"HDD Total {gc:.2f}, Delta {gtd:+.2f}. ST vs LT HDD divergence. AO={ao['now']}. Be brutal.")
    ...
    with t2:
        idx_c = st.columns(3); ids = [("AO", ao, "https://www.cpc.ncep.noaa.gov/products/precip/CWlink/daily_ao_index/ao.sprd2.gif", "Ispod -2.0: EXTREME BULLISH (Pucanje vrtloga)."),
